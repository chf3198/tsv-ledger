...
/**
 * File Size Management Optimization System
 * Advanced pre-check validation and automated refactoring for 300-line limit compliance
 */
...
if (require.main === module) {
    const optimizer = new FileSizeManagementOptimizer();
    optimizer.runFileSizeOptimization().then(result => {
        console.log('\n🎯 File Size Optimization Results:');
        console.log(`Total Files: ${result.metrics.totalFiles}`);
        console.log(`Compliant Files: ${result.metrics.compliantFiles}`);
        console.log(`Oversized Files: ${result.metrics.oversizedFiles}`);
        console.log(`Average Size: ${result.metrics.averageSize.toFixed(1)} lines`);
        console.log(`Compliance Rate: ${result.final_report.summary.complianceRate.toFixed(1)}%`);
        process.exit(0);
    }).catch(error => {
        console.error('File size optimization failed:', error);
        process.exit(1);
...
module.exports = FileSizeManagementOptimizer;