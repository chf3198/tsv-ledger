/**
 * Data Lifecycle Integration Tests (Standalone)
 *
 * Tests the full data lifecycle via API without global test setup/teardown.
 * This test runs in isolation and manages its own database state and server.
 *
 * @module tests/integration/data-lifecycle-standalone.test.js
 */
...
describe('Data Lifecycle Integration Tests (Standalone)', () => {
  let serverProcess;

  beforeAll(async () => {
    // Start the server manually
    console.log('Starting TSV Ledger server for standalone test...');
    ...
    const checkServer = () => {
      return new Promise((resolve, reject) => {
        const req = require('http').get(BASE_URL + '/health', (res) => {
          let data = '';
    ...
    while (!serverReady && (Date.now() - startTime) < startupTimeout) {
      try {
        await checkServer();
        serverReady = true;
  ...
  }, 60000); // 1 minute timeout for server startup
  ...
  test('should complete full data lifecycle', async () => {
    ...
    const expendituresCount = expendituresData.expenditures?.length || 0;
    expect(expendituresCount).toBeGreaterThan(0);
    console.log('Expenditures API count:', expendituresCount);

    // Test analytics API
    const analyticsResponse = await fetch(`${BASE_URL}/api/analysis`);
    expect(analyticsResponse.ok).toBeTruthy();
    const analyticsData = await analyticsResponse.json();
    console.log('Analytics data preview:', JSON.stringify(analyticsData).substring(0, 200));

    // Test category breakdown (using analysis endpoint which includes categories)
    const categoriesResponse = await fetch(`${BASE_URL}/api/analysis`);
    expect(categoriesResponse.ok).toBeTruthy();
    const categoriesData = await categoriesResponse.json();
    expect(categoriesData.categories).toBeDefined();

    // Test subscription analysis
    const subscriptionsResponse = await fetch(`${BASE_URL}/api/subscription-dashboard`);
    expect(subscriptionsResponse.ok).toBeTruthy();

    // Test geographic data
    const geographicResponse = await fetch(`${BASE_URL}/api/geographic-dashboard`);
    ...
    expect(backupStatusData.backups.length).toBeGreaterThan(0);
    console.log('Backups after clear:', backupStatusData.backups.length);
    ...
    const emptyExpendituresCount = emptyExpendituresData.expenditures?.length || 0;
    ...
    const emptyAnalyticsResponse = await fetch(`${BASE_URL}/api/analysis`);
    ...
    const emptySubscriptionsResponse = await fetch(`${BASE_URL}/api/subscription-dashboard`);
