...
(async () => {
  ...
  try {
    ...
    if (!ids || ids.length === 0) throw new Error('No business items found for multiselect test');
    ...
    if (counts.benefits < ids.length) throw new Error('Benefits count does not reflect multiselect selections');
