...
(async () => {
  ...
  try {
    ...
    const openBtn = await page.$(
      '[data-bs-toggle="modal"][data-bs-target="#employeeBenefitsModal"], #openBenefitsModal, .open-benefits-modal'
    );
    if (openBtn) {
      ...
      console.log(
        "No modal button found, invoking showSelectionModal if available"
      );
      await page.evaluate(() => {
        if (
          window.employeeBenefitsManager &&
          typeof window.employeeBenefitsManager.showSelectionModal ===
            "function"
        ) {
    ...
    await new Promise((resolve) => setTimeout(resolve, 300)); // allow UI to render

    // Count initial business cards
    const businessCount = await page.$$eval(
      "#businessSuppliesList .col-md-6",
      (els) => els.length
    );
    const benefitsCount = await page.$$eval(
      "#benefitsList .col-md-6",
      (els) => els.length
    );
    ...
    if (businessCount === 0 && benefitsCount === 0) {
      throw new Error(
        "No cards found in modal lists — smoke test cannot proceed"
      );
    }
    ...
    if (businessCount > 0) {
      const sampleTerm = await page.$eval(
        "#businessSuppliesList .col-md-6 .card-title",
        (el) => (el ? el.textContent.split(" ")[0].slice(0, 3) : "")
      );
      if (sampleTerm && sampleTerm.length > 0) {
        ...
        const businessFiltered = await page.$$eval(
          "#businessSuppliesList .col-md-6",
          (els) =>
            els.filter((e) => {
        ...
        console.log(
          "Could not compute representative sampleTerm for business search, skipping content assertion"
        );
      }
    } else {
      console.log(
        "No business cards present, skipping business search assertion"
      );
    }

    // Type a term into benefits search and assert filtered results decrease or equal
    if (benefitsCount > 0) {
      const sampleTerm2 = await page.$eval(
        "#benefitsList .col-md-6 .card-title",
        (el) => (el ? el.textContent.split(" ")[0].slice(0, 3) : "")
      );
      if (sampleTerm2 && sampleTerm2.length > 0) {
        ...
        await new Promise((resolve) => setTimeout(resolve, 300));
        const benefitsFiltered = await page.$$eval(
          "#benefitsList .col-md-6",
          (els) =>
            els.filter((e) => {
              const style = window.getComputedStyle(e);
        ...
        console.log(
          "Could not compute representative sampleTerm for benefits search, skipping content assertion"
        );
      }
    } else {
      console.log(
        "No benefits cards present, skipping benefits search assertion"
      );
    }
