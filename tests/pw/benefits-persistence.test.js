...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      if (typeof mgr.saveProgressiveAllocations === 'function') {
        mgr.saveProgressiveAllocations();
      }
      if (typeof mgr.saveSelection === 'function') {
        mgr.saveSelection();
      }
    });
    ...
    await page.waitForFunction(() => {
      try {
        const mgr = (typeof employeeBenefitsManager !== 'undefined') ? employeeBenefitsManager : (window.employeeBenefitsManager || null); return !!mgr && typeof mgr.loadSavedSelection === 'function';
      } catch (e) {
        return false;
      }
    }, { timeout: 20000 });
    ...
    const persisted = await page.evaluate(() => {
      const ls = (() => {
        try {
          return JSON.parse(localStorage.getItem('employeeBenefitsSelection') || '[]');
        } catch (e) {
          return [];
        }
      })();
    ...
    if (!persisted || (persisted.lsCount === 0 && persisted.mgrCount === 0)) {
      throw new Error('Selection did not persist across reload');
    }
