...
class TestFixtures {
  ...
  static get mockApiResponses() {
    return {
      ...
      analysis: {
        totalSpent: 1000.0,
        categories: { "Office Supplies": 500.0 },
        monthlyTrends: [],
        topVendors: [],
      },
      aiAnalysis: {
...
class PageHelpers {
  static async waitForAppLoad(page) {
    ...
    try {
      ...
      const navVisible = await page
        .isVisible("#main-navigation")
        .catch(() => false);
      if (width >= 768 && !navVisible) {
        try {
          await page.evaluate(() => {
            if (window.__ensureSidebarState) window.__ensureSidebarState(false);
          });
        } catch (e) {}
  ...
  static async navigateToSection(page, sectionId) {
    ...
    await page.evaluate((sid) => {
      const link = Array.from(document.querySelectorAll("[data-section]")).find(
        (a) => a.getAttribute("data-section") === sid
      );
      if (link) {
        try {
          link.click();
  ...
  static async waitForLoadingComplete(page) {
    await page.waitForFunction(
      () => {
        ...
        return (
          loaders.length === 0 ||
          Array.from(loaders).every((el) => el.style.display === "none")
        );
      },
      { timeout: 10000 }
    );
...
class AssertionHelpers {
  static async assertVisible(page, selector, message = "") {
    ...
    expect(isVisible, message || `Element ${selector} should be visible`).toBe(
      true
    );
  }
  ...
  static async assertHidden(page, selector, message = "") {
    ...
    expect(isHidden, message || `Element ${selector} should be hidden`).toBe(
      true
    );
  }
  ...
  static async assertApiResponse(response, expectedStatus = 200) {
    expect(response.status, `API should return status ${expectedStatus}`).toBe(
      expectedStatus
    );
    const contentType = response.headers.get("content-type");
    expect(contentType, "Response should be JSON").toContain(
      "application/json"
    );
  }
}
