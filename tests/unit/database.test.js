...
describe('Database Module', () => {
  const testDataFile = path.join(__dirname, '..', '..', 'src', 'expenditures.json');
  ...
  describe('addExpenditure', () => {
    ...
    test('should increment ID for multiple additions', (done) => {
      ...
      const expenditure2 = { ...TestFixtures.sampleExpenditure, id: undefined, description: 'Second item' };
        ...
        database.addExpenditure(exp, (err, added) => {
          ...
          if (completed === expenditures.length) {
            ...
            database.getAllExpenditures((err2, allExpenditures) => {
              ...
              expect(allExpenditures.map(e => e.description)).toEqual(['Item 1', 'Item 2', 'Item 3']);
              done();
            });
          }
        });
  ...
  describe('Error Handling', () => {
    test('should handle concurrent access gracefully', (done) => {
      const expenditure1 = { ...TestFixtures.sampleExpenditure, description: 'Concurrent 1' };
      const expenditure2 = { ...TestFixtures.sampleExpenditure, description: 'Concurrent 2' };

      // Simulate concurrent operations
      Promise.all([
        new Promise(resolve => database.addExpenditure(expenditure1, resolve)),
        new Promise(resolve => database.addExpenditure(expenditure2, resolve))
      ]).then(() => {
        database.getAllExpenditures((err, expenditures) => {
          expect(err).toBeNull();
