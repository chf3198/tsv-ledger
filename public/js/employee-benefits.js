...
class EmployeeBenefitsManager {
  constructor() {
    ...
    this.progressiveAllocationStorageKey =
      "employeeBenefitsProgressiveAllocations";
  ...
  async loadAmazonItems() {
    try {
      console.log(
        "🔍 Loading Amazon items for benefits filtering (business card only)..."
      );
      ...
      console.log(
        `📦 Loaded ${this.amazonItems.length} business card Amazon items`
      );
    } catch (error) {
    ...
    try {
      ...
      const normalizeEntry = (raw) => {
        if (!raw || typeof raw !== "object")
          return { benefits: 0, business: 100, total: 100 };
        ...
        if (benefits == null && raw.allocated != null)
          benefits = Number(raw.allocated);
        // Backwards-compat: older shape used 'remaining' for business
        if (business == null && raw.remaining != null)
          business = Number(raw.remaining);
      ...
      if (
        typeof allocs.forEach === "function" &&
        typeof allocs.get === "function" &&
        typeof allocs.set === "function"
      ) {
        // It's a Map
  ...
  createBusinessCard(item) {
    const alloc = this.itemProgressiveAllocations.get(item.id) || {
      benefits: 0,
      business: 100,
    };
    const businessAmount = (item.amount * alloc.business) / 100;

    const div = document.createElement("div");
    div.className = "col-md-6 col-lg-4";
    // expose searchable metadata for the modal search inputs
    div.dataset.itemName = (item.description || "").toLowerCase();
    div.dataset.itemId = item.id;
    div.innerHTML = `
      <div class="card h-100 border-info">
        <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
          <small class="fw-bold">${item.category || "Other"}</small>
          <button class="btn btn-sm btn-outline-light" onclick="employeeBenefitsManager.editItem('${
            item.id
          }')" title="Edit item">
        ...
        <div class="card-body">
          ...
          <div class="mb-2">
            <span class="badge bg-primary">$${businessAmount.toFixed(2)} (${
      alloc.business
    }%)</span>
          </div>
          <div class="mb-3">
            ...
            <div class="d-flex align-items-center gap-2">
              <input type="range" class="form-range flex-grow-1" min="0" max="100" value="${
                alloc.benefits
              }"
                     data-item-id="${
                       item.id
                     }" oninput="employeeBenefitsManager.updateAllocationSlider('${
      item.id
    }', this.value)">
  ...
  createBenefitsCard(item) {
    ...
    const benefitsAmount = (item.amount * alloc.benefits) / 100;
    ...
    div.innerHTML = `
      <div class="card h-100 border-success">
        ...
        <div class="card-body">
          <h6 class="card-title">${item.description}</h6>
          <div class="mb-2">
            <span class="badge bg-success">$${benefitsAmount.toFixed(2)} (${
      alloc.benefits
    }%)</span>
          </div>
          <div class="mb-3">
  ...
  updateAllocationSlider(itemId, benefitsPercentage) {
    ...
    console.log(
      `✅ Updated ${itemId}: ${businessPercentage}% business, ${pct}% benefits`
    );
  }
  ...
  resetAllAllocations() {
    if (
      confirm(
        "Are you sure you want to reset all allocations to 100% Business Supplies?"
      )
    ) {
      this.amazonItems.forEach((item) => {
        this.itemProgressiveAllocations.set(item.id, {
  ...
  updateModalItems() {
    ...
    const searchTerm =
      document.getElementById("searchAvailable")?.value.toLowerCase() || "";
    const searchSelectedTerm =
      document.getElementById("searchSelected")?.value.toLowerCase() || "";
    ...
    const selectedItemsArray = this.amazonItems.filter((item) => {
      ...
      return (
        progressiveAlloc.allocated > 0 &&
        (item.name.toLowerCase().includes(searchSelectedTerm) ||
          item.category.toLowerCase().includes(searchSelectedTerm))
      );
    });
    ...
    sortedCategories.forEach((categoryKey) => {
      ...
      const categoryRemainingValue = items.reduce((sum, item) => {
        const progressiveAlloc = this.itemProgressiveAllocations.get(
          item.id
        ) || { allocated: 0, remaining: 100 };
        const remainingAmount =
          item.price - (item.price * progressiveAlloc.allocated) / 100;
      ...
      header.innerHTML = `
        <div class="d-flex justify-content-between align-items-center">
          ...
          <div class="d-flex align-items-center gap-2">
            <small class="text-primary fw-bold">$${categoryRemainingValue.toFixed(
              2
            )}</small>
    ...
    if (availableItems.length > 0) {
      ...
      availableSummary.className =
        "list-group-item bg-primary bg-opacity-10 border-primary";
      ...
      `;
      availableContainer.insertBefore(
        availableSummary,
        availableContainer.firstChild
      );
    }
    ...
    if (selectedItemsArray.length === 0) {
      ...
      summary.className =
        "list-group-item bg-success bg-opacity-10 border-success";
      summary.innerHTML = `
        <div class="d-flex justify-content-between align-items-center">
          ...
          <span class="badge bg-success fs-6">
            ${selectedItemsArray.length} items • $${totalAllocatedValue.toFixed(
        2
      )}
          </span>
        </div>
      ...
      Object.keys(groupedSelected)
        .sort(
          (a, b) =>
            (categoryInfo[a]?.priority || 99) -
            (categoryInfo[b]?.priority || 99)
        )
        .forEach((categoryKey) => {
          ...
          const categoryTotal = items.reduce((sum, item) => {
            ...
            const allocatedAmount =
              (item.price * progressiveAlloc.allocated) / 100;
            return sum + allocatedAmount;
          }, 0);
  ...
  updateModalDisplay() {
    ...
    console.log(
      "📊 Updating modal display with",
      this.amazonItems.length,
      "items"
    );
    ...
    // Update counts
    document.getElementById("businessItemsCount").textContent =
      businessItems.length;
    document.getElementById("benefitsItemsCount").textContent =
      benefitsItems.length;

    console.log(
      `📊 Updated modal: ${businessItems.length} business items, ${benefitsItems.length} benefits items`
    );
  }
  ...
  quickSelectCategory(category) {
    const categoryItems = this.amazonItems.filter(
    ...
    console.log(
      `✅ Quick selected ${categoryItems.length} items in category: ${category}`
    );
  }
  ...
  selectCategoryItems(category) {
    ...
    console.log(
      `✅ Selected all ${categoryItems.length} items in category: ${category}`
    );
  }
  ...
  createItemElement(item, isSelected, categoryColor = "secondary") {
    ...
    div.className = `list-group-item list-group-item-action ${
      isSelected ? "selected-item" : "available-item"
    }`;
    div.dataset.itemId = item.id;
    ...
    if (isSelected) {
      ...
      priceDisplay = `$${allocatedAmount.toFixed(2)} (${
        progressiveAlloc.allocated
      }%)`;
    } else if (progressiveAlloc.allocated > 0) {
      // In available list but partially allocated - show remaining portion
      priceDisplay = `$${remainingAmount.toFixed(2)} (${
        progressiveAlloc.remaining
      }%)`;
    } else {
    ...
    const controlDescription = isSelected
      ? ""
      : `(select portion from remaining ${progressiveAlloc.remaining}%)`;
    const percentageControl = `
      <div class="mt-2 p-2 bg-light rounded">
        ...
        ${
          controlDescription
            ? `<small class="text-muted d-block mb-2">${controlDescription}</small>`
            : ""
        }
        <div class="d-flex align-items-center gap-2">
          <input type="range" class="form-range flex-grow-1 percentage-slider" min="0" max="${
            progressiveAlloc.remaining
          }" value="${progressiveAlloc.remaining}"
                 data-item-id="${item.id}"
                 oninput="employeeBenefitsManager.updateSliderDisplay('${
                   item.id
                 }', this.value)">
          <small class="text-muted percentage-display" style="min-width: 35px;">${
            progressiveAlloc.remaining
          }%</small>
          ${
            progressiveAlloc.allocated > 0
              ? `<button class="btn btn-outline-secondary btn-sm" onclick="employeeBenefitsManager.resetProgressiveAllocation('${item.id}')" title="Reset all allocations"><i class="fas fa-undo"></i></button>`
              : ""
          }
        </div>
        <div class="d-flex justify-content-between mt-1">
          <small class="text-success">Benefits: $${allocatedAmount.toFixed(
            2
          )}</small>
          <small class="text-muted">Remaining: $${remainingAmount.toFixed(
            2
          )}</small>
        </div>
        ${
          progressiveAlloc.allocated > 0
            ? `<div class="mt-1"><small class="text-info">Allocated: ${progressiveAlloc.allocated}% | Available: ${progressiveAlloc.remaining}%</small></div>`
            : ""
        }
      </div>
    `;
        ...
        <div class="flex-grow-1">
          ...
          <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="btn-group btn-group-sm">
              ...
              <button class="btn ${
                isSelected ? "btn-outline-danger" : "btn-outline-success"
              } add-to-benefits-btn"
                      data-item-id="${item.id}"
                      title="${
                        isSelected
                          ? "Remove from benefits"
                          : `Add current percentage to benefits`
                      }">
                <i class="fas ${isSelected ? "fa-minus" : "fa-plus"}"></i>
                <span class="button-text">${
                  isSelected ? "Remove" : `Add ${progressiveAlloc.remaining}%`
                }</span>
              </button>
            </div>
          </div>
  ...
  updateSummaryStats(businessItems, benefitsItems) {
    ...
    const businessValue = businessItems.reduce((sum, item) => {
      ...
      return sum + (item.amount * alloc.business) / 100;
    }, 0);
    ...
    const benefitsValue = benefitsItems.reduce((sum, item) => {
      ...
      return sum + (item.amount * alloc.benefits) / 100;
    }, 0);
    ...
    console.log("🔍 Updating summary stats:", {
      totalItems,
      businessValue: Number.isFinite(businessValue)
        ? businessValue.toFixed(2)
        : "NaN",
      benefitsValue: Number.isFinite(benefitsValue)
        ? benefitsValue.toFixed(2)
        : "NaN",
      benefitsPercentage: Number.isFinite(benefitsPercentage)
    ...
    if (businessValueEl)
      businessValueEl.textContent = `$${businessValue.toFixed(2)}`;
    if (benefitsValueEl)
      benefitsValueEl.textContent = `$${benefitsValue.toFixed(2)}`;
    if (benefitsPercentageEl)
      benefitsPercentageEl.textContent = `${benefitsPercentage.toFixed(1)}%`;
  }
  ...
  addItemWithPercentage(itemId) {
    ...
    const slider = itemElement
      ? itemElement.querySelector('input[type="range"]')
      : null;
  ...
  updateAddButtonText(itemId) {
    ...
    const availableElement = document.querySelector(
      `#availableItems [data-item-id="${itemId}"]`
    );
    const selectedElement = document.querySelector(
      `#selectedItems [data-item-id="${itemId}"]`
    );

    const progressiveAlloc = this.itemProgressiveAllocations.get(itemId) || {
      allocated: 0,
      remaining: 100,
    };
    ...
    if (availableElement) {
      ...
      if (button && buttonText) {
        if (progressiveAlloc.remaining > 0) {
          ...
          button.setAttribute(
            "onclick",
            `employeeBenefitsManager.updateProgressiveAllocation('${itemId}', ${percentage})`
          );
    ...
    if (selectedElement) {
      ...
      if (button && buttonText) {
        ...
        button.setAttribute(
          "onclick",
          `employeeBenefitsManager.resetProgressiveAllocation('${itemId}')`
        );
  ...
  updateProgressiveAllocation(itemId, additionalPercentage) {
    ...
    const currentAlloc = this.itemProgressiveAllocations.get(itemId) || {
      allocated: 0,
      remaining: 100,
    };
    ...
    if (pct > currentAlloc.remaining) {
      console.warn(
        `⚠️ Cannot allocate ${pct}% - only ${currentAlloc.remaining}% remaining`
      );
      return;
    }
    ...
    console.log(
      `✅ Added ${pct}% to ${itemId}: ${newAllocated}% allocated, ${newRemaining}% remaining`
    );
  }
  ...
  resetProgressiveAllocation(itemId) {
    ...
    console.log(
      `✅ Reset allocation for ${itemId} to 0% allocated, 100% remaining`
    );
  }
  ...
  updateSliderDisplay(itemId, percentage) {
    ...
    const itemElement = document.querySelector(`[data-item-id="${itemId}"]`);
    if (itemElement) {
      const percentageDisplay = itemElement.querySelector(
        ".percentage-display"
      );
      ...
      if (button && buttonText) {
        ...
        const isInAvailableList =
          itemElement.closest("#availableItems") !== null;
        ...
        if (isInAvailableList && progressiveAlloc.remaining > 0) {
          ...
          button.setAttribute(
            "onclick",
            `employeeBenefitsManager.updateProgressiveAllocation('${itemId}', ${pct})`
          );
    ...
    if (itemElement) {
      ...
      if (item) {
        ...
        const percentageText = itemElement.querySelector(
          'small.text-muted[style*="min-width"]'
        );
        if (percentageText) {
  ...
  filterItems(type, searchTerm) {
    ...
    const containerId =
      type === "business" ? "businessSuppliesList" : "benefitsList";
  ...
  clearSelection() {
    if (
      confirm(
        "Are you sure you want to clear all selected items and their allocations?"
      )
    ) {
  ...
  saveSelection() {
    try {
      ...
      console.log(
        `💾 Saved ${selectedArray.length} selected items to localStorage`
      );
    } catch (error) {
  ...
  saveProgressiveAllocations() {
    try {
      const progressiveObject = Object.fromEntries(
        this.itemProgressiveAllocations
      );
      localStorage.setItem(
        this.progressiveAllocationStorageKey,
        JSON.stringify(progressiveObject)
      );
      console.log(
        `💾 Saved ${this.itemProgressiveAllocations.size} progressive allocations to localStorage`
      );
    } catch (error) {
  ...
  loadSavedProgressiveAllocations() {
    try {
      ...
      if (saved) {
        ...
        this.itemProgressiveAllocations = new Map(
          Object.entries(progressiveObject)
        );
        // Normalize any legacy allocation shapes so rest of the code can assume a single schema
        this.normalizeProgressiveAllocations();
        console.log(
          `📁 Loaded ${this.itemProgressiveAllocations.size} progressive allocations from localStorage`
        );
      }
    } catch (error) {
  ...
  normalizeProgressiveAllocations() {
    try {
      ...
      this.itemProgressiveAllocations.forEach((val, key) => {
        ...
        if (
          benefits === null &&
          (obj.hasOwnProperty("allocated") || obj.hasOwnProperty("remaining"))
        ) {
  ...
  loadSavedSelection() {
    try {
      ...
      if (saved) {
        ...
        console.log(
          `📁 Loaded ${selectedArray.length} selected items from localStorage`
        );
      }
    } catch (error) {
  ...
  loadSavedAllocations() {
    try {
      ...
      if (saved) {
        ...
        console.log(
          `📁 Loaded ${this.itemAllocations.size} item allocations from localStorage`
        );
      }
    } catch (error) {
  ...
  async generateBenefitsReport() {
    try {
      ...
      console.log(
        "📊 Generating benefits report for",
        selectedIds.length,
        "items..."
      );
              ...
              <div class="list-group list-group-flush">
                ...
                <div class="list-group-item d-flex justify-content-between align-items-center">
                  ...
                  <span class="badge bg-info">${
                    allocation.averageAllocationPercentage
                  }%</span>
                </div>
                <div class="list-group-item d-flex justify-content-between align-items-center">
                  Benefits Amount
                  <span class="badge bg-success">$${allocation.totalBenefitsAmount.toFixed(
                    2
                  )}</span>
                </div>
                <div class="list-group-item d-flex justify-content-between align-items-center">
                  Operational Amount
                  <span class="badge bg-secondary">$${allocation.totalOperationalAmount.toFixed(
                    2
                  )}</span>
                </div>
              </div>
                  ...
                  <div class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-end">
                      ...
                      <small class="text-success">$${
                        data.benefitsAmount?.toFixed(2) || "0.00"
                      }</small>
                      <small class="text-muted">/ $${
                        data.operationalAmount?.toFixed(2) || "0.00"
                      }</small>
                    </div>
                  </div>
                `
              ...
              <table class="table table-sm">
                ...
                <tbody>
                  ${report.items
                    ...
                    <tr>
                      <td>${item.name}</td>
                      <td><small>${this.getCategoryLabel(
                        item.category
                      )}</small></td>
                      <td class="text-end">$${item.price.toFixed(2)}</td>
                      <td class="text-center">${item.allocationPercentage}%</td>
                      <td class="text-end text-success">$${item.benefitsAmount.toFixed(
                        2
                      )}</td>
                      <td class="text-end text-muted">$${item.operationalAmount.toFixed(
                        2
                      )}</td>
                    </tr>
                  `
  ...
  saveAndCloseModal() {
    this.saveSelection();
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("employeeBenefitsModal")
    );
    if (modal) {
      modal.hide();
    }
    this.showMessage(
      `Saved ${this.selectedItems.size} items as benefits`,
      "success"
    );
  }
  ...
  populateItemsDropdown() {
    ...
    console.log(
      "Legacy populateItemsDropdown called - using new modal interface"
    );
  }
  ...
  extractAmazonItems(analysisData) {
    console.log(
      "Legacy extractAmazonItems called - data now loaded directly from API"
    );
    return [];
  }
  ...
  quickSelectCategory(category) {
    ...
    setTimeout(() => {
      ...
      this.showMessage(
        `Selected all items in ${categoryName} category`,
        "success"
      );
    }, 300);
  }
  ...
  loadDemoData() {
    ...
    this.amazonItems = [
      {
        id: "demo-1",
        description: "Premium Coffee K-Cups",
        amount: 24.99,
        category: "employee_amenities",
        date: "2025-01-01",
      },
      {
        id: "demo-2",
        description: "Office Paper - White 8.5x11",
        amount: 15.49,
        category: "office_supplies",
        date: "2025-01-01",
      },
  ...
  updateModalDisplay() {
    ...
    this.amazonItems.forEach((item) => {
      const allocation = this.itemProgressiveAllocations.get(item.id) || {
        benefits: 0,
        business: 100,
      };
    ...
    if (businessCountEl)
      businessCountEl.textContent = `${businessItems.length} items`;
    if (benefitsCountEl)
      benefitsCountEl.textContent = `${benefitsItems.length} items`;
...
function _initBenefitsIfNeeded() {
  try {
    ...
    if (window.employeeBenefitsManager) {
      ...
      if (typeof window.employeeBenefitsManager.initialize === "function") {
        if (!window.employeeBenefitsManager.initialized) {
          window.employeeBenefitsManager
            ...
            .catch((err) =>
              console.error(
                "Error initializing benefits manager on SPA navigation:",
                err
              )
            );
        } else {
      ...
      try {
        console.warn(
          "Replacing stubbed employeeBenefitsManager with real instance"
        );
      } catch (e) {}
      ...
      window.employeeBenefitsManager
        ...
        .catch((err) =>
          console.error(
            "Error initializing benefits manager on SPA navigation (replaced stub):",
            err
          )
        );
      return;
    ...
    window.employeeBenefitsManager
      ...
      .catch((err) =>
        console.error(
          "Error initializing benefits manager on SPA navigation (new):",
          err
        )
      );
  } catch (e) {
  ...
  document.addEventListener(
    ...
    (ev) => {
      ...
      if (
        ...
        detail.toLowerCase().includes("benefit")
      ) {
        setTimeout(_initBenefitsIfNeeded, 50);
      }
    },
  ...
  if (!window.ensureBenefitsInitialized) {
    window.ensureBenefitsInitialized = async function () {
      try {
        ...
        if (
          !window.employeeBenefitsManager ||
          typeof window.employeeBenefitsManager.initialize !== "function"
        ) {
          window.employeeBenefitsManager = new EmployeeBenefitsManager();
        }
